...
const [setting,setSetting]=useState("café")
const [mood,setMood]=useState("romantic")
const [buildUp,setBuildUp]=useState("slow burn")
const [maleRole,setMaleRole]=useState("stranger")
const [femaleRole,setFemaleRole]=useState("traveler")
...
return name.replace("Microsoft ","").replace("Google ","").split(" - ")[0]
}
...
<select className="border p-3 rounded-lg" value={setting} onChange={(e)=>setSetting(e.target.value)}>
<option>café</option>
...
<select className="border p-3 rounded-lg" value={mood} onChange={(e)=>setMood(e.target.value)}>
...
<select className="border p-3 rounded-lg" value={buildUp} onChange={(e)=>setBuildUp(e.target.value)}>
...
<select className="border p-3 rounded-lg" value={maleRole} onChange={(e)=>setMaleRole(e.target.value)}>
<option>boss</option>
...
<select className="border p-3 rounded-lg" value={femaleRole} onChange={(e)=>setFemaleRole(e.target.value)}>
...
<select className="border p-3 rounded-lg" value={storyType} onChange={(e)=>setStoryType(e.target.value)}>
...
<select className="border p-3 rounded-lg" value={narratorVoice} onChange={(e)=>setNarratorVoice(e.target.value)}>
{englishVoices.map(v=>(
...
<select className="border p-3 rounded-lg" value={maleVoice} onChange={(e)=>setMaleVoice(e.target.value)}>
{englishVoices.map(v=>(
<option key={v.name} value={v.name}>
{cleanVoiceName(v.name)}
</option>
...
<select className="border p-3 rounded-lg" value={femaleVoice} onChange={(e)=>setFemaleVoice(e.target.value)}>
{englishVoices.map(v=>(
<option key={v.name} value={v.name}>
{cleanVoiceName(v.name)}
</option>
...
<button className="bg-black text-white p-3 rounded-lg" onClick={generateStory}>
Generate Story
</button>
...
<button className="bg-black text-white px-4 py-2 rounded-lg" onClick={speakStory}>
🔊 Listen
</button>

<button className="border px-4 py-2 rounded-lg" onClick={stopStory}>
Stop
</button>
